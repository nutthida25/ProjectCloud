  ...
  for (const key of Object.keys(config)) {
    if (key !== "MODE" && !(key in deprecationData) && !(key in warnedInvalidKeys)) {
      if (key.startsWith("COMPILER_")) {
        if (isRuntimeOnly()) {
          warn(`Deprecation config "${key}" is compiler-specific and you are running a runtime-only build of Vue. This deprecation should be configured via compiler options in your build setup instead.
Details: https://v3-migration.vuejs.org/breaking-changes/migration-build.html`);
  ...
  if (instance && config[
    ...
    warn(`Deprecation config "${"OPTIONS_DATA_MERGE"}" can only be configured globally.`);
  }
}
...
function createCompatVue$1(createApp2, createSingletonApp) {
  ...
  Vue2.version = `2.6.14-compat:${"3.2.47"}`;
...
//# sourceMappingURL=chunk-NO4IMCB4.js.map
